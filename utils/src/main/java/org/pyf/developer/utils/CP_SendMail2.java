...
public class CP_SendMail2 implements CP_ISendMail{
	...
	Logger logger = LoggerFactory.getLogger(CP_SendMail2.class);
	...
	public CP_SendMail2() {
		...
		setNeedAuth(true);
		setNamePass("nqcoopService2@netqin.com", "nq2011");
		setFrom("noreply@nqservice.com");
	}

	public CP_SendMail2(String smtp) {
		setSmtpHost(smtp);
		createMimeMessage();
	...
	public boolean sendout() {
		logger.info("sendMail2:sendout->begin");
		try {
			...
			transport.close();
			++sendCount;
			logger.info("mail2 sendCount:"+sendCount);
			return true;
		} catch (Exception e) {
			logger.error("邮件发送失败！" + e.toString());
			CP_SendMail1 s = new CP_SendMail1();
	...
	public boolean sendout(MimeMessage  mimeMsg) {
		logger.info("sendMail2:sendoutByMimeMessage->begin");
		try {
			Session mailSession = Session.getInstance(props, null);
			Transport transport = mailSession.getTransport("smtp");
			transport.connect((String) props.get("mail.smtp.host"), username,password);
	...
	public static void main(String[] args) {
		CP_SendMail2 sm = new CP_SendMail2();
		//sm.setFrom("hanqunfeng@netqin.com");
		
		sm.setSubject("主题123");
		sm.setBody("内容123,邱思湲");
		String[] mails = {"hanqunfeng@nq.com"};
		sm.setTo(mails);
		String[] mails2 = {"songge@nq.com"};
		//sm.setCopyTo(mails2);
...
//				String[] mails3 = {"hanqunfeng@nq.com"};
//				sm.setTo(mails3);
//				if (sm.sendout()) {
...
//				}
			...
			System.out.print("sendmail " + ex.toString());
		}
	}
}
