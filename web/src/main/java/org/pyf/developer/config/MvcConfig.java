...
public class MvcConfig implements WebMvcConfigurer {
    ...
    //    log.info("CP_ReturnHandler");
    //    //因为自定义的拦截器会被最后执行，所以对于拦截返回值是String类型的情况是不生效的
    //    handlers.add(new CP_ReturnHandler());
    //}
    ...
    /**
     * 启用thymeleaf时关闭jsp视图处理器，其实可以共存的，设置好order就行，只是没必要在一个项目中存在两种页面处理方式
     * @return
     */
    @Bean
