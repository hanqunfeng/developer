$(function(){
	...
	$("#delete").click(function(){
		if($("#resultsTable tbody input:checkbox:checked[name='ids']").length<=0){
			bootbox.alert("<font color\='red'>Please Choose One Item ！</font>");
		}else{
			bootbox.confirm("<font color\='red'>Confirm Delete？</font>",function(result){
				if(result){
					$("#resultForm").submit();
				}
			});
	...
	if($("#resultsTable").find("tbody tr").length == 0){
		...
		$("<tr><td colspan='" + tdlen + "'>No Record <a href='#'>Add</a></td></tr>").
